...
def init_db():
    ...
    if conn.execute("SELECT COUNT(*) FROM listings").fetchone()[0] == 0:
        sample_listings = [
            ...
            # ... all other listings with sample_landlord_id added
        ]
        conn.executemany(
...
def home():
    ...
    if category == 'All' or category == '':
        listings = conn.execute('SELECT * FROM listings ORDER BY id DESC LIMIT 6').fetchall()
    else:
        listings = conn.execute('SELECT * FROM listings WHERE type = ? ORDER BY id DESC LIMIT 6', (category,)).fetchall()
    
    conn.close()
    return render_template('home.html', listings=listings, categories=CATEGORY_LABELS, selected_category=category)
...
def listings():
    ...
    if q:
        ...
        for term in terms:
            ...
            conditions.append('(lower(title) LIKE ? OR lower(location) LIKE ? OR lower(type) LIKE ? OR lower(description) LIKE ?)')
            params.extend([wildcard, wildcard, wildcard, wildcard])
        sql = 'SELECT * FROM listings WHERE ' + ' AND '.join(conditions) + ' ORDER BY id DESC'
        listings = conn.execute(sql, params).fetchall()
    else:
        listings = conn.execute('SELECT * FROM listings ORDER BY id DESC').fetchall()
    conn.close()
    return render_template('listings.html', listings=listings, query=q)
...
def student_start():
    conn = get_db_connection()
    listings = conn.execute('SELECT id, title, type, location FROM listings ORDER BY id DESC LIMIT 10').fetchall()
    conn.close()
    ...
    if request.method == 'POST':
        ...
        if student:
            ...
            if preferred_listing:
                flash(f'Your dorm request for "{preferred_listing["title"]}" has been sent to the owner.', 'success')
            else:
                flash('Student preferences saved. You can now browse dorms and contact landlords.', 'success')
            return redirect(url_for('listings'))
